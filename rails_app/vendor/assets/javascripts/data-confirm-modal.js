...
(function ($) {
  ...
  // Detect bootstrap version. BS5 defers $.fn.modal registration to
  // DOMContentLoaded, so we detect lazily and fall back to 5.
  var bootstrapVersion = 5;
  function detectBootstrapVersion() {
    if ($.fn.modal && $.fn.modal.Constructor && $.fn.modal.Constructor.VERSION) {
      var match = $.fn.modal.Constructor.VERSION.match(/^(\d)\./);
      if (match) bootstrapVersion = parseInt(match[1]);
    }
  }
  ...
  var buildModal = function (options) {
    ...
    var modalClose = '<button type="button" class="btn-close" data-bs-dismiss="modal" aria-hidden="true">'+modalCloseContent+'</button>'
    ...
    $('.modal.show, .modal.in').not('#'+id).each(function() {
    ...
    modal.spawn = function() {
      // BS5's jQuery adapter doesn't auto-show from config object;
      // initialize with config then explicitly call 'show'.
      modal.modal($.extend({}, {
