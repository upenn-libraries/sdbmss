...
class AccountsController < SearchableAuthorityController
  ...
  def model_params
    params.require(model_class_lstr.to_sym).permit(:username, :fullname, :institutional_affiliation, :email, :email_is_public, :password, :password_confirmation, :role, :bio, :active, :notification_setting_attributes => [:on_update, :on_comment, :on_reply, :on_message, :email_on_update, :email_on_comment, :email_on_reply, :email_on_message])
  end
