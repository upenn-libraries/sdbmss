class ApplicationController < ActionController::Base
  ...
  def configure_permitted_parameters
    ...
    devise_parameter_sanitizer.for(:account_update) { |u| u.permit(:username, :email, :email_is_public, :fullname, :institutional_affiliation, :password, :password_confirmation, :current_password, :bio, :notification_setting_attributes => [:on_update, :on_comment, :on_reply, :on_message, :on_new_user, :on_all_comment, :email_on_new_user, :email_on_update, :email_on_comment, :email_on_reply, :email_on_message, :email_on_all_comment]) }
  end
