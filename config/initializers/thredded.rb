...
# overrides notification job to use in-app notifications - a better way to do this is probably being described above (^)
module Thredded
    ...
    def perform(post_id)
      ...
      post.postable.followers.each do |follower|
        if follower != post.user
          follower.notify("#{post.user} has posted in #{post.postable.title}", post, "forum_post")
        end
      end
    end
  end
